...
describe('constants', () => {
  ...
  it('namespaces the query-parameter keys to avoid clashes with caller URLs', () => {
    expect(IWPC_WINDOW_ID_QUERY_PARAM.startsWith('__iwpc')).toBe(true);
    expect(IWPC_PARENT_ID_QUERY_PARAM.startsWith('__iwpc')).toBe(true);
    expect(IWPC_WINDOW_ID_QUERY_PARAM).not.toBe(IWPC_PARENT_ID_QUERY_PARAM);
  });
});
