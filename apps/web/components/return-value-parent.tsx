...
type Props = {
  transport: Transport;
  childRoute: string;
  /** Shown in the WindowFrame badge. */
  transportLabel: 'postMessage' | 'BroadcastChannel';
  /** Single-paragraph description shown above the action buttons. */
  intro: React.ReactNode;
  parentSnippet: string;
  childSnippet: string;
};

export function ReturnValueParentBody({
  transport,
  childRoute,
  transportLabel,
  intro,
  ...
  const runRemote = useCallback(
    async <T,>(
      ...
      setResult(null);
      try {
        const child = await iwpcWindow.open(
          `${childRoute}?kind=${kind}`,
          OPEN_OPTS
        );
        if (!child) {
    ...
    >
      ...
      <Card className='bg-card/60 backdrop-blur'>
        ...
        <CardContent className='flex flex-col gap-3'>
          <CodeBlock code={parentSnippet} filename='parent.tsx' />
          <CodeBlock code={childSnippet} filename='child.tsx' />
        </CardContent>
      </Card>
    </WindowFrame>
  );
